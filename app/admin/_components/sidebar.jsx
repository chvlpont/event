...
function Sidebar({ setSelectedPage }) {
  ...
  const handlePageSelect = (page) => { 
    ...
    setActivePage(page);
    if (page === 'CreateEvent' || page === 'EditEvent') {
  ...
  return (
    <div className={`h-screen bg-gray-800 text-white flex flex-col items-start justify-start pt-5 transition-all duration-500 ease-in-out ${isCollapsed ? 'w-20' : 'w-64'}`}>
      <div className="flex items-center w-full mb-2 p-4 justify-between">
        {!isCollapsed && <h1 className="h-8 w-auto text-white text-center text-2xl font-bold tracking-wide ">NexusPanel</h1>} 
        {isCollapsed 
...
<div className={`cursor-pointer flex items-center w-full mb-2 hover:bg-gray-700 p-4 ${activePage === 'Event' ? 'bg-gray-700 text-white border-l-4 border-red-500' : ''}`} onClick={() => { handlePageSelect('Event'); setShowEventOptions(!showEventOptions); }} onKeyDown={(event) => handleKeyDown(event, () => { handlePageSelect('Event'); setShowEventOptions(!showEventOptions); })} tabIndex="0" role="button" aria-label="Event">
      ...
      {showEventOptions && (
        <>
<div className={`cursor-pointer flex items-center w-full mb-2 hover:bg-gray-700 p-4 pl-8 ${activePage === 'CreateEvent' ? 'bg-gray-700 text-white border-l-4 border-green-500' : ''}`} onClick={() => handlePageSelect('CreateEvent')} onKeyDown={(event) => handleKeyDown(event, () => handlePageSelect('CreateEvent'))} tabIndex="0" role="button" aria-label="Create Event">
  <FaCalendarPlus className="text-white mr-2" />
  {!isCollapsed && <p className="text-white">Create Event</p>}
</div>
<div className={`cursor-pointer flex items-center w-full mb-2 hover:bg-gray-700 p-4 pl-8 ${activePage === 'EditEvent' ? 'bg-gray-700 text-white border-l-4 border-yellow-500' : ''}`} onClick={() => handlePageSelect('EditEvent')} onKeyDown={(event) => handleKeyDown(event, () => handlePageSelect('EditEvent'))} tabIndex="0" role="button" aria-label="Edit Event">
  <FaEdit className="text-white mr-2" />
  {!isCollapsed && <p className="text-white">Edit Event</p>}
</div>
        </>
      )}
